...
//        dataTrain = this.removeParticularAttributes(dataTrain);
    ...
    public void runTopology(String pathTestDirectory) throws Exception {
        ...
        Double probNormal = 0.85d;
        Double probAttack = 0.85d;
        ...
        for (int i = 0; i < this.testFiles.size(); i++) {
            ...
            if (i >= delayDays) {
                ...
                for (Instance inst : dataUpdate) {
                    tree.updateClassifier(inst);
                }

                ((FilteredClassifier) inputMapped.getClassifier()).setClassifier(tree);
                ((InputMappedClassifier) classifier).setClassifier((FilteredClassifier) inputMapped.getClassifier());
            }
